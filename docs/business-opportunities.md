...
- **AtRisk** = current score 3 or 4, **or** the two most recent score changes are both downgrades and both within the last 90 days. **Watch** = at least one downgrade within the last 90 days (and not AtRisk). **Ok** = otherwise. A downgrade is a move to a higher score number (1 = best).
- **The panel lists a location if any of:** current score 3/4; a downgrade within 90 days (any decline, e.g. 1→2 — the main gap in the old rule); or no score yet ("awaiting first inspection"). No score yet is a *reason*, not a risk level (there is nothing to judge). Delisted establishments are never flagged.
...
- A location that dipped and has since recovered still lists for 90 days (it *did* decline in the window); if that proves noisy, the rule to revisit is "declined and not yet back at its previous score".
...
- "Needs attention" redesign and shared RiskLevel: AtRisk = score 3/4 or two consecutive downgrades both within 90 days; Watch = one downgrade within 90 days; panel also lists locations with no score yet; "overdue for reinspection" dropped for lack of cadence data (2026-09-20)
