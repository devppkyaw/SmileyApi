...
- **`/locations.html`** — the old Locations tab: add/remove by CVR, group by CVR, embed code, history. Now server-paged and server-searched (`GET /v1/business/locations?page&pageSize&q&sort`), so a chain with hundreds of locations no longer ships every inspection row to the browser. **Group by CVR** lists every CVR as a collapsed header with its location count (`groupsOnly=1`, no rows), and opening a CVR loads its locations 25 at a time with "Show more" (`cvr=<cvr>`, or `cvr=none` for locations without one); a list of 3 or fewer CVRs opens automatically. (Plain row paging split one big CVR across many pages, so only one group was ever visible.)
