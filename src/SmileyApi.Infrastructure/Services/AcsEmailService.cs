...
public class AcsEmailService(IConfiguration config, ILogger<AcsEmailService> logger) : IEmailService
{
    ...
    private string Sender =>
        config["Acs:SenderAddress"] ?? "donotreply@smiley.dk";
    ...
    public async Task SendVerificationEmailAsync(string to, string verifyUrl, CancellationToken ct = default)
    {
        var message = new EmailMessage(
            ...
            recipientAddress: to,
            content: new EmailContent("Verify your SmileyAPI account")
            {
                Html = $"<p>Click the link below to verify your email and activate your SmileyAPI account:</p>" +
                       $"<p><a href=\"{verifyUrl}\">{verifyUrl}</a></p>" +
                       "<p>This link expires in 24 hours.</p>",
                PlainText = $"Verify your SmileyAPI account:\n{verifyUrl}\n\nThis link expires in 24 hours."
            });
        ...
        var message = new EmailMessage(
            ...
            content: new EmailContent($"Smiley score update: {establishmentName}")
            {
                Html = $"<p>The smiley score for <strong>{establishmentName}</strong> has changed.</p>" +
                       ...
                       "<p>Log in to your <a href=\"https://smiley.dk/dashboard.html\">SmileyAPI dashboard</a> for details.</p>",
