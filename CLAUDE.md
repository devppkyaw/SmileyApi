...
## Pull Request Workflow (updated 2026-09-20)
All work happens on ONE long-lived branch, **`dev`**. Do not create a new branch per change or per PR — the user runs the app locally and fixes things in one place.

1. **Work on `dev`.** Make every fix and feature there. Commit only your own files — never `.claude/`, `docs/Smiley_xml_*.xml` or `src/SmilrApi.Api/wwwroot/launch.html`, and don't stage `CLAUDE.md` or `docs/business-opportunities.md` without re-reading them first (see Multi-Session Editing). If `origin/main` has moved on, bring it in with `git fetch` + `git merge origin/main` (merge, not rebase, so pushed history is never rewritten).
2. **Open the PR from `dev`, don't merge it.** Push `dev` and `gh pr create --base main --head dev`. If a PR from `dev` is already open, pushing updates it — don't open another. The user merges it (`gh pr merge` is blocked in auto mode); merging to `main` deploys to production, so the PR description should say what was and wasn't tested.
3. **Report local state on request or after a merge.** When the user asks whether local is up to date, or says a PR merged: `git fetch --prune`, check `gh pr view <n> --json state`, and report `dev` vs `origin/dev` and local `main` vs `origin/main` (ahead/behind).
4. **After the PR shows `MERGED`, sync — standing permission, no need to ask again:** `git checkout main`, `git pull --ff-only`, `git checkout dev`, `git merge main`, then report the resulting state. Keep working on `dev`.
5. **Never delete `dev`, locally or on GitHub** — it is meant to live on. Also leave alone `customer-domain-test` and its worktree `C:\Projects\Smilr-customer-domain-test`, any other branches, the stash, and untracked files — mention them instead. If the PR isn't `MERGED` yet, don't sync; just say so.
